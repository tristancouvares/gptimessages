...
def text_bot(
    ...
    while True:
        print("Waiting...")from imessage_reader import fetch_data
from chat_functionality import *
from config import *
import subprocess
from tokenizers import ByteLevelBPETokenizer
...
def truncate_conversation(conversation):
    while count_tokens(json.dumps(conversation)) > MAX_TOKENS:
        tokens_to_remove = sum(
            count_tokens(conversation[i]["content"]) + 1 for i in range(0, 2)
        )
